...
namespace luminous {
    ...
    inline namespace matrix {
        ...
        struct Matrix3x3 {
            ...
            XPU _NODISCARD vector_t &operator[](Index i) noexcept {
                return cols[i];
            }

            template<typename Index>
            XPU _NODISCARD constexpr vector_t operator[](Index i) const noexcept {
                return cols[i];
            }
...
#define _define_matrix3x3(type)                                                                          \
        ...
        XPU _NODISCARD constexpr auto make_##type##3x3(type val = 1) {                                \
            return type##3x3(val);                                                                       \
        }                                                                                                \
        XPU _NODISCARD constexpr auto make_##type##3x3(type##3x3::vector_t c0,                        \
                                                            type##3x3::vector_t  c1,                     \
                                                            type##3x3::vector_t  c2) noexcept {          \
            return type##3x3{c0, c1, c2};                                                                \
        }                                                                                                \
        XPU _NODISCARD constexpr auto make_##type##3x3(                                               \
        ...
        XPU _NODISCARD constexpr auto make_##type##3x3(const Matrix4x4<type> m) noexcept {            \
...
#define _define_matrix4x4(type)                                                                         \
        ...
        XPU _NODISCARD constexpr auto make_##type##4x4(type val = 1) noexcept {                      \
            return type##4x4{val};                                                                      \
        }                                                                                               \
        XPU _NODISCARD constexpr auto make_##type##4x4(type##4x4::vector_t c0,                       \
                                        type##4x4::vector_t c1,                                         \
                                        type##4x4::vector_t c2,                                         \
                                        type##4x4::vector_t c3) noexcept {                              \
            return type##4x4{c0, c1, c2, c3};                                                           \
        }                                                                                               \
        XPU _NODISCARD constexpr auto make_##type##4x4(                                              \
        ...
        XPU _NODISCARD constexpr auto make_##type##4x4(const type##3x3 m) noexcept {                 \
...
#undef _define_matrix4x4
        ...
        template<typename T>
        XPU _NODISCARD constexpr Vector<T, 3> operator*(const Matrix3x3<T> m, Vector<T, 3> v) noexcept {
            return v.x * m[0] + v.y * m[1] + v.z * m[2];
        }

        template<typename T>
        XPU _NODISCARD constexpr Matrix4x4<T> operator*(const Matrix4x4<T> lhs, T v) noexcept {
            return Matrix4x4<T>(v * lhs[0], v * lhs[1], v * lhs[2], v * lhs[3]);
        }

        template<typename T>
        XPU _NODISCARD constexpr Matrix4x4<T> operator*(T v, const Matrix4x4<T> rhs) noexcept {
            return Matrix4x4<T>(v * rhs[0], v * rhs[1], v * rhs[2], v * rhs[3]);
        }

        template<typename T>
        XPU _NODISCARD constexpr Matrix3x3<T> operator*(const Matrix3x3<T> lhs, T v) noexcept {
            return Matrix3x3<T>(v * lhs[0], v * lhs[1], v * lhs[2]);
        }

        template<typename T>
        XPU _NODISCARD constexpr Matrix3x3<T> operator*(T v, const Matrix3x3<T> rhs) noexcept {
            return Matrix3x3<T>(v * rhs[0], v * rhs[1], v * rhs[2]);
        }

        template<typename T>
        XPU _NODISCARD constexpr Matrix3x3<T> operator*(const Matrix3x3<T> lhs, const Matrix3x3<T> rhs) noexcept {
            return Matrix3x3<T>(lhs * rhs[0], lhs * rhs[1], lhs * rhs[2]);
        }

        template<typename T>
        XPU _NODISCARD constexpr Vector<T, 4> operator*(const Matrix4x4<T> m, Vector<T, 4> v) noexcept {
            return v.x * m[0] + v.y * m[1] + v.z * m[2] + v.w * m[3];
        }

        template<typename T>
        XPU _NODISCARD constexpr Matrix4x4<T> operator*(const Matrix4x4<T> lhs, const Matrix4x4<T> rhs) noexcept {
