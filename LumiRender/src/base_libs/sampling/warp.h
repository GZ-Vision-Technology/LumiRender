...
namespace luminous {
    inline namespace sampling {
        ...
        LM_ND_XPU_INLINE float2 square_to_disk(const float2 u) {
            auto r = sqrt(u.x);
            auto theta = constant::_2Pi * u.y;
            return make_float2(r * cos(theta), r * sin(theta));
        }

        LM_ND_XPU_INLINE float uniform_disk_PDF() {
            return constant::invPi;
        }

        LM_ND_XPU_INLINE float3 square_to_cosine_hemisphere(const float2 u) {
            auto d = square_to_disk(u);
            auto z = sqrt(std::max(0.0f, 1.0f - d.x * d.x - d.y * d.y));
            return make_float3(d.x, d.y, z);
        }

        LM_ND_XPU_INLINE float cosine_hemisphere_PDF(float cos_theta) {
            return cos_theta * constant::invPi;
        }

        LM_ND_XPU_INLINE float3 square_to_cone(const float2 u, float cos_theta_max) {
        ...
        LM_ND_XPU_INLINE float uniform_cone_PDF(float cos_theta_max) {
            return 1 / (constant::_2Pi * (1 - cos_theta_max));
        }

        LM_ND_XPU_INLINE float2 square_to_triangle(const float2 u) {
            auto su0 = sqrt(u.x);
            return make_float2(1 - su0, u.x * su0);
        }

        LM_ND_XPU_INLINE float3 square_to_sphere(float2 u) {
            float z = 1 - 2 * u[0];
        ...
        LM_ND_XPU_INLINE float uniform_sphere_PDF() {
         ...
         * @return
         */
        LM_ND_XPU_INLINE float PDF_dir(float PDF_pos, float3 normal, float3 wo) {
            float cos_theta = abs_dot(normal, normalize(wo));
            return PDF_pos * length_squared(wo) / cos_theta;
        }

        LM_ND_XPU_INLINE float3 square_to_hemisphere(const float2 sample) {
            float z = sample.x;
        ...
        LM_ND_XPU_INLINE float uniform_hemisphere_PDF() {
            return constant::inv2Pi;
        }

        LM_ND_XPU_INLINE float balance_heuristic(int nf,
                                                 float f_PDF,
                                                 int ng,
                                                 float g_PDF) {
        ...
        LM_ND_XPU_INLINE float power_heuristic(int nf,
                                               float f_PDF,
                                               int ng,
                                               float g_PDF) {
        ...
        LM_ND_XPU_INLINE float MIS_weight(int nf,
                                          float f_PDF,
                                          int ng,
                                          float g_PDF) {
        ...
        LM_ND_XPU_INLINE float MIS_weight(float f_PDF, float g_PDF) {
