...
namespace luminous {
    inline namespace sampling {
        ...
        ND_XPU_INLINE float2 square_to_disk(const float2 u) {
            auto r = sqrt(u.x);
            auto theta = constant::_2Pi * u.y;
            return make_float2(r * cos(theta), r * sin(theta));
        }

        ND_XPU_INLINE float uniform_disk_PDF() {
            return constant::invPi;
        }

        ND_XPU_INLINE float3 square_to_cosine_hemisphere(const float2 u) {
            auto d = square_to_disk(u);
            auto z = sqrt(std::max(0.0f, 1.0f - d.x * d.x - d.y * d.y));
            return make_float3(d.x, d.y, z);
        }

        ND_XPU_INLINE float cosine_hemisphere_PDF(float cos_theta) {
            return cos_theta * constant::invPi;
        }

        ND_XPU_INLINE float3 square_to_cone(const float2 u, float cos_theta_max) {
        ...
        ND_XPU_INLINE float uniform_cone_PDF(float cos_theta_max) {
        ...
        ND_XPU_INLINE float uniform_sphere_PDF() {
        ...
        ND_XPU_INLINE float uniform_hemisphere_PDF() {
            return constant::inv2Pi;
        }

        ND_XPU_INLINE float balance_heuristic(int nf,
                                              float f_PDF,
                                              int ng,
                                              float g_PDF) {
        ...
        ND_XPU_INLINE float power_heuristic(int nf,
                                            float f_PDF,
                                            int ng,
                                            float g_PDF) {
        ...
        ND_XPU_INLINE float MIS_weight(int nf,
                                       float f_PDF,
                                       int ng,
                                       float g_PDF) {
        ...
        ND_XPU_INLINE float MIS_weight(float f_PDF, float g_PDF) {
