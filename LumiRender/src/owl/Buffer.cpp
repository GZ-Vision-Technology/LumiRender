...
namespace owl {
    ...
    void DeviceBuffer::DeviceDataForTextures::executeResize() {
        SetActiveGPU forLifeTime(device);
        if (d_pointer) {
            CUDA_CALL(Free(d_pointer));
            d_pointer = nullptr;
        }

        if (parent->elementCount) CUDA_CALL(Malloc(&d_pointer, parent->elementCount * sizeof(cudaTextureObject_t)));
    ...
    void HostPinnedBuffer::resize(size_t newElementCount) {
        ...
        elementCount = newElementCount;
        if (newElementCount > 0) CUDA_CALL(MallocHost((void **) &cudaHostPinnedMem, sizeInBytes()));
