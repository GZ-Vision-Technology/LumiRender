...
namespace luminous::render {
    inline namespace warp {
        ...
        inline float2 square_to_disk(const float2 u) {
            auto r = sqrt(u.x);
            auto theta = constant::_2Pi * u.y;
            return make_float2(r * cos(theta), r * sin(theta));
        }

        inline float uniform_disk_pdf() {
            return constant::invPi;
        }

        inline float3 square_to_cosine_hemisphere(const float2 u) {
        ...
        inline float3 square_to_cone(const float2 u, float cos_theta_max) {
        ...
        inline float2 square_to_triangle(const float2 u) {
            auto su0 = sqrt(u.x);
            return make_float2(1 - su0, u.x * su0);
        }

        inline float3 square_to_sphere(float2 u) {
            float z = 1 - 2 * u[0];
        ...
        inline float3 square_to_hemisphere(const float2 sample) {
            float z = sample.x;
