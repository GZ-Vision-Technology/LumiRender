...
namespace owl {
    namespace common {
        ...
        struct interval {
            ...
            inline __both__ interval<T> including(const T &t) { return interval<T>(min(lower, t), max(upper, t)); }
        ...
        template<typename T>
        inline __both__ interval<T> build_interval(const T &a, const T &b) { return interval<T>(min(a, b), max(a, b)); }
        ...
        template<typename T>
        inline __both__ bool operator!=(const interval<T> &a, const interval<T> &b) { return !(a == b); }
        ...
        struct box_t {
            ...
            inline __both__ typename long_type_of<typename T::scalar_t>::type
            volume() const { return owl::common::volume(size()); }
        ...
        template<typename T>
        inline __both__ typename long_type_of<T>::type area(const box_t<vec_t < T, 2>>
    ...
    template<typename T>
    inline __both__ bool operator!=(const box_t <T> &a, const box_t <T> &b) { return !(a == b); }
