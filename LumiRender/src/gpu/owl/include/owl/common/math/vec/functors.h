...
namespace owl {
    namespace common {
        ...
        inline __both__
        bool any_less_than(const vec_t <T, N> &a, const vec_t <T, N> &b) { return common::any(lt(a, b)); }

        template<typename T, int N>
        inline __both__
        bool all_less_than(const vec_t <T, N> &a, const vec_t <T, N> &b) { return common::all(lt(a, b)); }

        template<typename T, int N>
        inline __both__
        bool any_greater_than(const vec_t <T, N> &a, const vec_t <T, N> &b) { return common::any(gt(a, b)); }

        /*! particularly useful for doing some range testing as in "if
          (any_greater_or_equal(pixelID,frameBufferSize)" etc */
        template<typename T, int N>
        inline __both__
        bool any_greater_or_equal(const vec_t <T, N> &a, const vec_t <T, N> &b) { return common::any(ge(a, b)); }
        ...
        template<typename T>
        inline __both__ T clamp(const T &val, const T &lo, const T &hi) { return min(hi, max(lo, val)); }

        template<typename T>
        inline __both__ T clamp(const T &val, const T &hi) { return clamp(val, (T) 0, hi); }
...
#undef _define_operator
        ...
        template<typename T>
        inline __both__ vec_t<T, 2> operator-(const vec_t<T, 2> &v) { return vec_t<T, 2>(-v.x, -v.y); }

        template<typename T>
        inline __both__ vec_t<T, 2> operator+(const vec_t<T, 2> &v) { return vec_t<T, 2>(v.x, v.y); }

        template<typename T>
        inline __both__ vec_t<T, 3> operator-(const vec_t<T, 3> &v) { return vec_t<T, 3>(-v.x, -v.y, -v.z); }
