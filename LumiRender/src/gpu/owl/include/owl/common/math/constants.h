...
namespace owl {
    namespace common {
        ...
        struct limits_traits<true> {
            template<typename T>
            static inline __both__ T value_limits_lower(T) { return std::numeric_limits<T>::min(); }

            template<typename T>
            static inline __both__ T value_limits_upper(T) { return std::numeric_limits<T>::max(); }
        };

        template<>
        struct limits_traits<false> {
            template<typename T>
            static inline __both__ T
            value_limits_lower(T) { return (T) NegInfTy(); }//{ return -std::numeric_limits<T>::infinity(); }
            template<typename T>
            static inline __both__ T
            value_limits_upper(T) { return (T) PosInfTy(); }//{ return +std::numeric_limits<T>::infinity();  }
        };

        /*! lower value of a completely *empty* range [+inf..-inf] */
        template<typename T>
        inline __both__ T empty_bounds_lower() {
            return limits_traits<std::numeric_limits<T>::is_integer>::value_limits_upper(T());
        }

        /*! upper value of a completely *empty* range [+inf..-inf] */
        template<typename T>
        inline __both__ T empty_bounds_upper() {
            return limits_traits<std::numeric_limits<T>::is_integer>::value_limits_lower(T());
        }

        /*! lower value of a completely *empty* range [+inf..-inf] */
        template<typename T>
        inline __both__ T empty_range_lower() {
            return limits_traits<std::numeric_limits<T>::is_integer>::value_limits_upper(T());
        }

        /*! upper value of a completely *empty* range [+inf..-inf] */
        template<typename T>
        inline __both__ T empty_range_upper() {
            return limits_traits<std::numeric_limits<T>::is_integer>::value_limits_lower(T());
        }

        /*! lower value of a completely open range [-inf..+inf] */
        template<typename T>
        inline __both__ T open_range_lower() {
            return limits_traits<std::numeric_limits<T>::is_integer>::value_limits_lower(T());
        }

        /*! upper value of a completely open range [-inf..+inf] */
        template<typename T>
        inline __both__ T open_range_upper() {
            return limits_traits<std::numeric_limits<T>::is_integer>::value_limits_upper(T());
        }

        template<>
        inline __both__ uint16_t empty_bounds_lower<uint16_t>() { return uint16_t(USHRT_MAX); }

        template<>
        inline __both__ uint16_t empty_bounds_upper<uint16_t>() { return uint16_t(0); }

        template<>
        inline __both__ uint16_t open_range_lower<uint16_t>() { return uint16_t(0); }

        template<>
        inline __both__ uint16_t open_range_upper<uint16_t>() { return uint16_t(USHRT_MAX); }

        template<>
        inline __both__ int16_t empty_bounds_lower<int16_t>() { return int16_t(SHRT_MAX); }

        template<>
        inline __both__ int16_t empty_bounds_upper<int16_t>() { return int16_t(SHRT_MIN); }

        template<>
        inline __both__ int16_t open_range_lower<int16_t>() { return int16_t(SHRT_MIN); }

        template<>
        inline __both__ int16_t open_range_upper<int16_t>() { return int16_t(SHRT_MAX); }

        template<>
        inline __both__ uint8_t empty_bounds_lower<uint8_t>() { return uint8_t(CHAR_MAX); }

        template<>
        inline __both__ uint8_t empty_bounds_upper<uint8_t>() { return uint8_t(CHAR_MIN); }

        template<>
        inline __both__ uint8_t open_range_lower<uint8_t>() { return uint8_t(CHAR_MIN); }

        template<>
        inline __both__ uint8_t open_range_upper<uint8_t>() { return uint8_t(CHAR_MAX); }

        template<>
        inline __both__ int8_t empty_bounds_lower<int8_t>() { return int8_t(SCHAR_MIN); }

        template<>
        inline __both__ int8_t empty_bounds_upper<int8_t>() { return int8_t(SCHAR_MAX); }

        template<>
        inline __both__ int8_t open_range_lower<int8_t>() { return int8_t(SCHAR_MAX); }

        template<>
        inline __both__ int8_t open_range_upper<int8_t>() { return int8_t(SCHAR_MIN); }
