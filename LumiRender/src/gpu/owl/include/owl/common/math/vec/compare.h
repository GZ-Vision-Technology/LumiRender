...
namespace owl {
    namespace common {
        ...
        template<typename T>
        inline __both__ bool operator==(const vec_t<T, 2> &a, const vec_t<T, 2> &b) { return a.x == b.x && a.y == b.y; }
        ...
        inline __both__ bool operator!=(const vec_t <T, N> &a, const vec_t <T, N> &b) { return !(a == b); }
        ...
        -> vec_t<decltype(!a.x), 2> { return {!a.x, !a.y}; }

        template<typename T>
        inline __both__ auto nt(const vec_t<T, 3> &a)
        -> vec_t<decltype(!a.x), 3> { return {!a.x, !a.y, !a.z}; }

        template<typename T>
        inline __both__ auto nt(const vec_t<T, 4> &a)
        -> vec_t<decltype(!a.x), 4> { return {!a.x, !a.y, !a.z, !a.w}; }
        ...
        -> vec_t<decltype(a.x == b.x), 2> { return {a.x == b.x, a.y == b.y}; }

        template<typename T>
        inline __both__ auto eq(const vec_t<T, 3> &a, const vec_t<T, 3> &b)
        -> vec_t<decltype(a.x == b.x), 3> { return {a.x == b.x, a.y == b.y, a.z == b.z}; }

        template<typename T>
        inline __both__ auto eq(const vec_t<T, 4> &a, const vec_t<T, 4> &b)
        -> vec_t<decltype(a.x == b.x), 4> { return {a.x == b.x, a.y == b.y, a.z == b.z, a.w == b.w}; }
        ...
        -> decltype(nt(eq(a, b))) { return nt(eq(a, b)); }
        ...
        -> decltype(nt(lt(b, a))) { return nt(lt(b, a)); }
        ...
        -> decltype(nt(lt(a, b))) { return nt(lt(a, b)); }
        ...
        inline __both__ vec_t<T, 2> select(const vec_t<bool, 2> &mask,
                                           const vec_t<T, 2> &a,
                                           const vec_t<T, 2> &b) { return {mask.x ? a.x : b.x, mask.y ? a.y : b.y}; }
